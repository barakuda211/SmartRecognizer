...
namespace AForge.WindowsForms
{
    ...
    internal class MagicEye
    {
        ...
        private string processSample(ref Imaging.UnmanagedImage unmanaged)
        {
            ...
            rez = rects.Length.ToString() + " | ";

            for (int i = 0; i < rects.Length; ++i)
            {
                rez += " | " + (((double)rects[i].Width / (double)rects[i].Height)).ToString();
            }
            ...
            int ly = unmanaged.Width;
            int rx = 0;
            int ry = 0;
            for (int i = 0; i < rects.Length; ++i)
            {
                if (lx > rects[i].X) lx = rects[i].X;
                if (ly > rects[i].Y) ly = rects[i].X;
                if (rx < rects[i].X + rects[i].Width) rx = rects[i].X + rects[i].Width;
                if (ry < rects[i].Y + rects[i].Height) ry = rects[i].X + rects[i].Width;
            }
